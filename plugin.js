(function() {
  ...
  function component(object) {
    ...
    // ... (далі йде решта component() реалізації)
  }
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      lampac_voice_subscribe: { //
        uk: 'Підписатися на Говорун-версію'
      },
   ...
   Lampa.Template.add('lampac_css', "\n        <style>\n        @charset 'UTF-8';.online-prestige{position:relative;-webkit-border-radius:.3em;border-radius:.3em;background-color:rgba(0,0,0,0.3);display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex}.online-prestige__body{padding:1.2em;line-height:1.3;-webkit-box-flex:1;-webkit-flex-grow:1;-moz-box-flex:1;-ms-flex-positive:1;flex-grow:1;position:relative}@media screen and (max-width:480px){.online-prestige__body{padding:.8em 1.2em}}.online-prestige__img{position:relative;width:13em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;min-height:8.2em}.online-prestige__img>img{position:absolute;top:0;left:0;width:100%;height:100%;-o-object-fit:cover;object-fit:cover;-webkit-border-radius:.3em;border-radius:.3em;opacity:0;-webkit-transition:opacity .3s;-o-transition:opacity .3s;-moz-transition:opacity .3s;transition:opacity .3s}.online-prestige__img--loaded>img{opacity:1}@media screen and (max-width:480px){.online-prestige__img{width:7em;min-height:6em}}.online-prestige__folder{padding:1em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}.online-prestige__folder>svg{width:4.4em !important;height:4.4em !important}.online-prestige__viewed{position:absolute;top:1em;left:1em;background:rgba(0,0,0,0.45);-webkit-border-radius:100%;border-radius:100%;padding:.25em;font-size:.76em}.online-prestige__viewed>svg{width:1.5em !important;height:1.5em !important}.online-prestige__episode-number{position:absolute;top:0;left:0;right:0;bottom:0;display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-webkit-align-items:center;-moz-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-webkit-justify-content:center;-moz-box-pack:center;-ms-flex-pack:center;justify-content:center;font-size:2em}.online-prestige__loader{position:absolute;top:50%;left:50%;width:2em;height:2em;margin-left:-1em;margin-top:-1em;background:url(./img/loader.svg) no-repeat 50% 50%;background-size:contain}\n        /* additional minimal styles for lists and empty states */\n        .online-empty{padding:2em;text-align:center}\n        .torrent-list{padding:0.5em}\n        .lampac-balanser-loader{display:inline-block;vertical-align:middle}\n        </style>\n    ");
    ...
    Lampa.Template.add('lampac_prestige_full', '\
      <div class=\"online-prestige\">\
        <div class=\"online-prestige__img\"><img src=\"{img}\"/></div>\
        <div class=\"online-prestige__body\">\
          <div class=\"online-prestige__title\">{title}</div>\
          <div class=\"online-prestige__info\">{info}</div>\
          <div class=\"online-prestige__timeline\">{timeline}</div>\
        </div>\
      </div>');

    // resetTemplates — очищає кастомні шаблони при повторному запуску
    function resetTemplates() {
      // можна додати очистку/відновлення шаблонів, якщо потрібно
    }

    // Завершальна частина старту плагіна і його реєстрації
    try {
      if (!window.Lampa) window.Lampa = {};
      startPlugin();
    } catch (e) {
      console.error('Ошибка инициализации плагина By Ромашка v1.0', e);
    }

  // Закриваємо основну обгортку IIFE
})();
    ...
    /**
     * Показ того, що вибрано в фільтрі
     */
    ...
    this.draw = function(items) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          var voice_name = choice.voice_name || (filter_find.voice[0] ? filter_find.voice[0].title : false) || element.voice_name || (serial ? 'Невідомо' : element.text) || 'Невідомо';
          if (element.quality) {
    ...
    // Додаткові шаблони/іконки (коротко, але необхідні шаблони)
    Lampa.Template.add('lampac_prestige_folder', '\
      <div class="online-prestige online-prestige--folder">\
        <div class="online-prestige__img online-prestige__folder">{img}</div>\
        <div class="online-prestige__body">\
          <div class="online-prestige__title">{title}</div>\
          <div class="online-prestige__info">{info}</div>\
          <div class="online-prestige__timeline">{timeline}</div>\
    ...
    Lampa.Template.add('lampac_prestige_rate', '<span class="rate">{rate}</span>');

    // Іконки
    Lampa.Template.add('icon_viewed', '<svg viewBox="0 0 24 24" width="16" height="16"><path d="M12 5c-7 0-11 7-11 7s4 7 11 7 11-7 11-7-4-7-11-7zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10z"/></svg>');

    // Фіналізація: якщо додаток підтримує синхронізацію — налаштуємо sync для довготривалих ключів
    try {
      ...
      // якщо щось пішло не так зі sync — мовчимо, щоб не ламати старт плагіна
      console.warn('Sync init error', e);
    }

  } // кінець блоку startPlugin (якщо був відкритий)
  // Якщо плагін ще не стартував — стартуємо
  if (!window.bwarch_plugin) startPlugin();

  // Закриваємо глобальну обгортку IIFE
})();
