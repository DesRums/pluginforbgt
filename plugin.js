(function() {
  ...
  // --- Helpers & hostkey ---
  var hostkey = (Defined.localhost || '').replace('http://', '').replace('https://', '').replace(/\/+$/, '');
  ...
  // --- RCH (native websocket / proxy) setup ---
  ...
  function rchTypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) {
        ...
        // If testing on local host - try github as a "good" host
        var probe = Defined.localhost.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check';
        net.silent(probe, function() {
          check(true);
        }, function() {
          check(false);
        }, false, { dataType: 'text' });
  ...
  function RchRegistry(client, startConnection) {
    rchTypeInvoke(Defined.localhost, function() {
      client.invoke("RchRegistry", JSON.stringify({
        version: 149,
        host: location.host,
        rchtype: Lampa.Platform.is('android') ? 'apk' : Lampa.Platform.is('tizen') ? 'cors' : (window.rch_nws[hostkey] ? window.rch_nws[hostkey].type : ''),
  ...
  // ensure rch type resolved early
  rchTypeInvoke(Defined.localhost, function() {});
  ...
  function rchInvoke(json, call) {
    ...
    if (window.nwsClient[hostkey] && window.nwsClient[hostkey].socket) {
      try { window.nwsClient[hostkey].socket.close(); } catch (e) {}
    }
    ...
    window.nwsClient[hostkey].on('Connected', function(connectionId) {
      RchRegistry(window.nwsClient[hostkey], function() {
        call();
      });
    });
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient === 'undefined') {
      Lampa.Utils.putScript([Defined.localhost + "js/nws-client-es5.js?v18112025"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  // --- account URL builder (robust) ---
  function account(url) {
    url = url + '';
    // use Lampa.Utils.addUrlComponent if present (safer)
    try {
      var email = Lampa.Storage.get('account_email');
      if (email && url.indexOf('account_email=') === -1) {
        if (Lampa.Utils && Lampa.Utils.addUrlComponent) url = Lampa.Utils.addUrlComponent(url, 'account_email=' + encodeURIComponent(email));
        else url += (url.indexOf('?') >= 0 ? '&' : '?') + 'account_email=' + encodeURIComponent(email);
      }
    } catch (e) {}
    ...
    try {
      ...
      if (uid && url.indexOf('uid=') === -1) {
        if (Lampa.Utils && Lampa.Utils.addUrlComponent) url = Lampa.Utils.addUrlComponent(url, 'uid=' + encodeURIComponent(uid));
        else url += (url.indexOf('?') >= 0 ? '&' : '?') + 'uid=' + encodeURIComponent(uid);
      }
    } catch (e) {}

    try {
      var nws_id = (window.rch_nws && window.rch_nws[hostkey] ? window.rch_nws[hostkey].connectionId : '') || '';
      if (nws_id && url.indexOf('nws_id=') === -1) {
        if (Lampa.Utils && Lampa.Utils.addUrlComponent) url = Lampa.Utils.addUrlComponent(url, 'nws_id=' + encodeURIComponent(nws_id));
        else url += (url.indexOf('?') >= 0 ? '&' : '?') + 'nws_id=' + encodeURIComponent(nws_id);
      }
    } catch (e) {}

    // token intentionally left empty (reserved)
    return url;
  }
  ...
  function component(object) {
    ...
    // Load balansers with search capability on init if not already loaded
    if (typeof balansers_with_search === 'undefined') {
    ...
    // Clarification search storage (JSON-safe)
    ...
    this.initialize = function() {
      ...
      this.externalids().then(function() {
        ...
        try {
          if (Array.isArray(balansers_with_search) && !balansers_with_search.find(function(b) { return balanser && (balanser.slice(0, b.length) == b); })) {
          ...
          _this5.getFileUrl(item, function(json, json_call) {
            if (json && json.url) {
              ...
              if (first.url) {
                ...
                // Keep original online.js placeholder logic
                if (element.url) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          _this8.contextMenu({
            html: html,
            element: element,
            onFile: function(onFileCall) {
              if (params.onContextMenu) params.onContextMenu(element, html, data, onFileCall);
            },
  ...
  // --- helper to convert balanser item to a name key ---
  ...
  // --- Source search registration helper ---
  ...
  function startPlugin() {
    ...
    // CSS + templates (kept same as original)
    ...
    function addButton(e) {
      ...
      btn.on('hover:enter', function() {
        ...
        var all = Lampa.Storage.get('clarification_search', '{}');
        try { all = JSON.parse(all); } catch (er) { all = {}; }
